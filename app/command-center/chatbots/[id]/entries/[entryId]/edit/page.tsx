...
export default async function EditKBEntryPage({ params }: { params: { id: string; entryId: string } }) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/command-center/login')

  const [{ data: chatbot }, { data: entry }] = await Promise.all([
    supabase.from('chatbots').select('id, name').eq('id', params.id).single(),
    supabase.from('chatbot_kb_entries').select('*').eq('id', params.entryId).single(),
  ])
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <KBEntryForm chatbotId={params.id} entry={entry} />
    </div>
  )
}
