...
|----------|------|--------|-------|
| **0 ⭐** | **TechKit — technical superadmin (monitoring + stats for all MDN projects)** | **Sessions A + B COMPLETE + LIVE (Session 35, 2026-07-11)** | **Next: Session C — costs** of `command-center/TECHKIT-BRIEF.md` §13: Anthropic **Admin API** key (not the standard key) + `task=costs` collector (Anthropic HIGH, Railway/Vercel best-effort, Supabase static-config fallback) + `techkit/costs` page + manual-entry form + default cost alert rules (daily Anthropic > $5) — **closes priority 8**. Then Session D (AI weekly digest). Session B shipped providers/stats/deploy-feed (deploy feed via polling, not webhooks — Vercel webhooks are Pro-only); CRUX wired but origins low-traffic; `techkit-webhook` deployed dormant. MarketKit (0b) reuses TechKit's `_shared/` + notify, now proven. |
| **0b ⭐** | **MarketKit — AI GTM copilot as portal module (app.mdntech.org)** | **Sessions A + B-core LIVE (Session 38, 2026-07-11) — weekly sprint loop + screenshot metrics deployed; A7 CLOSED** | Loop is live and self-running: Mon 06:00 UTC review + 07:00 propose crons (migration 013), worker v2 (`sprint_propose`/`sprint_review`/`metrics_screenshot` + Telegram summaries), Sprint tab (approve/skip/done + re-roll), Metrics tab (screenshot vision import + manual entry). M9 via plain UTM `mk_links` per action (Dub = B3). First sprint proposed: 9 actions / 3 projects, Slovak for Royal Stroje. **[Martin] this week:** approve/skip proposals, upload first metrics screenshots (Monday's review needs data). **Next — Session B remainder:** B1 GA4/GSC ingestion (**[Martin]** create a Google Cloud service account + grant it GA4/GSC access on the dogfood properties), B3 Dub tracked links (**[Martin]** Dub account, free tier), B5 onboard remaining dogfood set (ChatKit, Melichárek, Good Hair by Zane). Then C1 content studio / C2 GEO module. Don't execute the MarketKit 30-day calendar before the D1 public-surface decision (open Q1). Data split: GA4/GSC/content/attribution → MarketKit; uptime/providers/costs → TechKit. Don't run Supabase go-lives concurrently with a live TechKit session. |
