...
| **0b ⭐** | **MarketKit — AI GTM copilot as portal module (app.mdntech.org)** | **Session A code-complete + build-verified (Session 36, 2026-07-11) — go-live pending** | Migration `012_marketkit.sql`, `marketkit-worker` Edge Function, `/portal/marketkit` module all built & green. **Next: run `command-center/MARKETKIT-SETUP.md`** (6 steps): apply migration 012 (after TechKit's 011); **[Martin]** sign up as a *customer* with an email ≠ his admin `team_members` email (open Q5) + SQL-enrol `marketkit`/`internal`; **[Martin]** add `ANTHROPIC_API_KEY` to edge secrets (`CRON_SECRET` reused, already set); deploy `marketkit-worker` via the multipart endpoint (`index.ts`+`claude.ts`+`_shared/supabase.ts`); smoke-test intake→scan→Launch Kit; A7 dogfood = MarketKit itself. **Then Session B** (BRIEF §7): metrics ingestion (GA4/GSC/screenshots), Dub tracked links, weekly sprint loop, onboard the rest of the dogfood set. Data split: GA4/GSC/content/attribution → MarketKit; uptime/providers/costs → TechKit. Don't run the Supabase go-live concurrently with a live TechKit session. |
