...
| **0b ⭐** | **MarketKit — AI GTM copilot as portal module (app.mdntech.org)** | **Session A LIVE (Session 37, 2026-07-11) — worker v1 deployed, 2 projects scanned, 1 Launch Kit generated; A7 nearly done** | Go-live complete: migration 012 applied + audited, `ANTHROPIC_API_KEY` in edge secrets, `marketkit-worker` v1 ACTIVE, dogfood `bugiiiii@protonmail.com` enrolled (`marketkit`/`internal`), E2E smoke green. Storage-RLS upload bug fixed live (`objects.name` qualification, `26ad3b9`). **Next: finish A7** — **[Martin]** retry asset upload (fix live, no deploy needed), re-run scan → profile v2, regenerate Launch Kit v2, create **MarketKit itself** as the third project (its kit = our GTM plan = A7 exit criterion). **Then Session B** (BRIEF §7): metrics ingestion (GA4/GSC/screenshots), Dub tracked links, weekly sprint loop, onboard the rest of the dogfood set. Data split: GA4/GSC/content/attribution → MarketKit; uptime/providers/costs → TechKit. Don't run Supabase go-lives concurrently with a live TechKit session. |
