...
**Phase B shipped (Session 23).** Analytics moved inline into chatbot detail, `/portal/dashboard` deleted, conversations restyled. Design consistency pass landed alongside (`button-primary` everywhere, dashboard rework, chip-style back links). Post-auth landing reversed from `/toolkit` → `/chatkit` (overrides Session 21 — see [decisions.md](decisions.md) Session 23 entry). User browser-tested in production and confirmed it looks good. Phase C (Settings restyle + marketing top-bar + Tools link) is next, then ChatKit monetization (pricing → auto-learning → voice).

| Priority | Task | Status | Notes |
|----------|------|--------|-------|
| 1 | Phase C -- Settings restyle + marketing top-bar swap + Tools link | Pending | App-shell on `/portal/settings`. Marketing site `components/main/navbar.tsx` (and Header equivalents): swap "Start Project" button for auth-aware Login/Portal button. Add a small Tools link/card pointing to `/portal/toolkit` either in the navbar or homepage Project section. Preconnect tags + cache-control on blackhole video for cross-origin smoothness between mdntech.org ↔ app.mdntech.org. |
| 2 | Production smoke test on `app.mdntech.org` | Partial | User confirmed chatbot detail + login → ChatKit redirect work. Still to verify: (a) signup confirmation email link goes to `/chatkit`, (b) `/toolkit` still renders public for logged-out visitors, (c) admin → portal cross-host rejection still works. |
| 3 | ChatKit empty-state marketing landing (when customer has no chatbots) | Pending (deferred from this session) | Per Session 23 discussion: render marketing-style hero on `/portal/chatkit` empty state explaining "knowledge base + 1 line of code = chatbot", with embed-snippet preview, coming-soon teasers (Voice / Auto-learning / Pricing tiers). Hybrid shell — marketing variant on empty state, app-shell when chatbots exist. Was paused to focus on dashboard rework + post-auth pivot first. |
...
| 11 | Delete `.next-stale-1777403470/` (local only) | Pending | Renamed mid-session to recover dev server; safety hook blocks `rm -rf .*`. Now in `.gitignore`. Delete manually with `rmdir /s /q .next-stale-1777403470` (Windows) or via Explorer. |
