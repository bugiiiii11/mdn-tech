...
**Session 25 shipped a full ChatKit + Settings design pass.** The Session 24 priority 1 ("ChatKit empty-state marketing landing") expanded by user direction into a complete portal-wide design unification: marketing shell with starfield on every ChatKit + Settings surface, gradient hero on `/chatkit`, BuildKBGuide 3-step onboarding (install Claude Code → paste KB-generation prompt → upload as KB entries), translucent backdrop-blur cards across the board, and the last gradient submit button replaced with `button-primary`. Production smoke test passed. **Next focus: Resend SMTP migration + ship a `/build-kb` skill so the inline KB-generation prompt also lives as a proper installable Claude Code skill (parallel work -- dashboard/DNS for Resend, code for the skill).**

| Priority | Task | Status | Notes |
|----------|------|--------|-------|
| 1 | Resend SMTP migration | Pending | Built-in Supabase email service has hard rate limits (~3-4/h free, ~30/h paid) and ships from `noreply@mail.app.supabase.io` — not viable once social drives signup volume. Switch to Resend: free 3000/mo, $20/mo for 50k. Steps: (a) sign up at resend.com, (b) add `mdntech.org` domain + DKIM/SPF DNS records, (c) create sending API key, (d) Supabase → Auth → Emails → SMTP Settings: host `smtp.resend.com`, port `465`, user `resend`, pass `<API key>`, sender `noreply@mdntech.org`, name `M.D.N Tech`, (e) test signup. Pure dashboard + DNS — no code change. Can run in parallel with priority 2. |
| 2 | `/build-kb` Claude Code skill | Pending | Surfaces the BuildKBGuide step-2 prompt as an installable Claude Code skill. Drop a `SKILL.md` into `.claude/skills/build-kb/`, add a `/build-kb` invocation handler that runs the project scan + writes `knowledge-base.md`, then add a card to `/portal/toolkit` (filter no longer hides this id). Followup of Session 25 design pass. |
| 3 | ChatKit Pricing + Stripe billing | Deferred | Per Session 20 plan. Migration 007 (stripe fields, plan enum), `/pricing` on marketing, `/portal/upgrade` in portal, caps 50/2000/10000/unlimited. Stripe account creation pending on user side; UI + schema can proceed in parallel. |
...
| 9 | SEO action plan | Pending | Follow `seo-audit/ACTION-PLAN.md` after Resend + `/build-kb` land. |
