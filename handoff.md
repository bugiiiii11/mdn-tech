...
**Session 26 closed the Resend SMTP migration (Session 25 priority 1) and tidied the marketing nav along the way.** Confirmation emails now arrive from `noreply@mdntech.org` via Resend with full DKIM/SPF/MX/DMARC, and the four other Supabase auth email templates (reset-password, magic-link, email-change, reauthentication) are version-controlled in `supabase/email-templates/` and live in the dashboard, ready for when those flows are wired in the portal UI. Marketing nav lost the `Tools` link and the Login CTA (hidden until the portal is publicly ready); nav pill recentered. **Next focus: priority 1 -- `/build-kb` Claude Code skill (was priority 2 last session).**

| Priority | Task | Status | Notes |
|----------|------|--------|-------|
| 1 | `/build-kb` Claude Code skill | Pending | Surfaces the BuildKBGuide step-2 prompt as an installable Claude Code skill. Drop a `SKILL.md` into `.claude/skills/build-kb/`, add a `/build-kb` invocation handler that runs the project scan + writes `knowledge-base.md`, then add a card to `/portal/toolkit` (filter no longer hides this id). Followup of Session 25 design pass. |
| 2 | ChatKit Pricing + Stripe billing | Deferred | Per Session 20 plan. Migration 007 (stripe fields, plan enum), `/pricing` on marketing, `/portal/upgrade` in portal, caps 50/2000/10000/unlimited. Stripe account creation pending on user side; UI + schema can proceed in parallel. |
| 3 | ChatKit Auto-Learning MVP | Deferred (resumes after Pricing) | Flag UI in conversation viewer + Sunday cron + Claude analysis + weekly report at `/portal/chatkit/[id]/learning`. Uses existing `message_feedback` table. ~5 hours. |
| 4 | ChatKit Voice via Cartesia Sonic-3 | Deferred (resumes after Auto-Learning) | Cartesia SDK in widget + API, voice toggle per chatbot, mic + audio playback, streaming TTS. $10/mo Pro add-on. ~6 hours. |
| 5 | Re-enable marketing Login/Portal CTA | Pending (gated on portal-public readiness) | When ChatKit pricing + a real public onboarding sequence ship, restore the auth-aware CTA in `components/main/navbar.tsx` (revert pieces of `643118a`): `isLoggedIn` prop + `ctaLabel`/`ctaHref` vars + desktop button JSX + mobile menu CTA JSX, and the `getUser()` fetch in `(marketing)/layout.tsx`. Keep the nav-pill absolute-center pattern from `643118a` (works either way). |
| 6 | Wire UI for the 4 branded auth flows | Pending | Templates ready (reset-password, magic-link, email-change, reauthentication). Each needs UI: "Forgot password?" link on login + a `/portal/reset-password` set-new-password page; magic-link option on login; email-change form in `/portal/settings`; reauth challenge surface for sensitive actions. Schedule per-flow as those features actually become needed. |
...
| 10 | SEO action plan | Pending | Follow `seo-audit/ACTION-PLAN.md` after `/build-kb` lands. |
| 11 | Delete `.next-stale-1777403470/` (local only) | Pending | Renamed in Session 23 to recover dev server; safety hook blocks `rm -rf .*`. Now in `.gitignore`. Delete manually with `rmdir /s /q .next-stale-1777403470` (Windows) or via Explorer. |
