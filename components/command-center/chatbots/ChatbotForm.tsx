...
export function ChatbotForm({ chatbot, projects }: { chatbot?: any; projects: { id: string; name: string }[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let result
    if (chatbot) {
      result = await supabase.from('chatbots').update(payload).eq('id', chatbot.id)
    } else {
    ...
    router.push(`/command-center/chatbots/${id}`)
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5 max-w-xl">
      <div className="bg-[#0d0d20] border border-white/5 rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.type} onChange={e => set('type', e.target.value)} className={inp}>
              ...
              <option value="internal">Internal chatbot</option>
            </select>
          </div>
          <div>
        ...
        <div>
          ...
          <input value={form.client_name} onChange={e => set('client_name', e.target.value)} placeholder="Client or company name" className={inp} />
        </div>

        <div>
          <label className="block text-xs text-gray-400 mb-1">Linked project (optional)</label>
